...
% 
% figure;clf; hold on;
% plot(x_test(:,1),y_test,'ko');
% plot(x_test(:,1),f(x_test),'r*');
...
q_trans = @(x,i) (x(i) - 0.5*(UB(i)+LB(i)))./(0.5.*(UB(i)-LB(i)));
P0 = @(x,i) 1;
P1 = @(x,i) q_trans(x,i);
P2 = @(x,i) (0.5)*(3.*q_trans(x,i).^2 - 1);
P3 = @(x,i) (0.5)*(5.*q_trans(x,i).^3 - 3.*q_trans(x,i));

Pfunc_vec = @(x,i,j) (i==0)*P0(x,j)+(i==1)*P1(x,j)+(i==2)*P2(x,j)+(i==3)*P3(x,j);
...
for i=0:max_poly
    for j=0:max_poly
        if i+j<=max_poly
            f = @(x) f(x) + coeff(i+j+1).*Pfunc_vec(x,j,1).*Pfunc_vec(x,i,2);
            disp([j,i])
        end
    end
